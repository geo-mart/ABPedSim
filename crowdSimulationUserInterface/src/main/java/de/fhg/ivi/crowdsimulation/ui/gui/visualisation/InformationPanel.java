...
public class InformationPanel extends JPanel implements Runnable
{
    ...
    private static final Logger logger              = LoggerFactory
        .getLogger(InformationPanel.class);
     ...
     */
    public InformationPanel(CrowdSimulation crowdSimulation)
    {
        ...
        logger.trace("InformationPanel(), height: " + height);

        // to avoid warnings (at the moment)
    ...
    /**
     * Thread for {@link InformationPanel} class. Thread is running if
     * {@link InformationPanel#infoThreadRunning}} is true.
     * <p>
    ...
    public void run()
    {
        ...
        while (infoThreadRunning)
        {
            ...
            crowdDensityLabel.setText("average crowd density= " + density / crowdSimulator.getCrowds().size()
                + " persons/m², total pedestrians=" + size + " in " + crowdSimulator.getCrowds().size()
                + " crowd(s)");
            ...
            {
                double simulationStepsPerSecond = 1 / (crowdSimulator.getSimulationUpdateInterval());
                ...
                graphicRefreshLabel.setText("graphic refreshs/second="
                    + Math.round(mapPanel.getGraphicRefreshsPerSecond()));
            }
