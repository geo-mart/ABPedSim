...
/**
 * Test implementation of new WayFinding Model, not working yet. Small changes, but more or less
 * copy of FollowWayPointsMentalModel
 *
 * @author Martin Knura
 *
 */
public class RoutingModel extends WayFindingModel
{
     ...
     */
    public RoutingModel(List<WayPoint> wayPoints, Vector2D initialPosition,
    ...
    private void checkNextWayPoint(Vector2D position)
    {
        ...
        if (visitedWayPoints.size() == wayPoints.size())
            ...
            else
            {
                ...
                wayPoints.add(this.getNextFromAgenda());
            }
        else
        {
    ...
    public List<WayPoint> getAgenda()
    {
        return agenda;
    }

    public void setAgenda(List<WayPoint> agenda)
    {
        this.agenda = agenda;
    }

    public WayPoint getNextFromAgenda()
    {
        WayPoint toSend = agenda.get(0);
        agenda.remove(0);
        return toSend;

    }

    // private void routingToTarget()
    // {
    // // Start & Ziel definieren
    //
    // Point pointstart = GeometryTools.coordinateToPoint(
    // new Coordinate(this.initialPositionVector.x(), this.initialPositionVector.y()));
    // Point pointtarget = GeometryTools.coordinateToPoint(
    // new Coordinate(this.currentDestinationWayPoint.x, this.currentDestinationWayPoint.y));
    // // System.out.println(pointstart.toString());
    // // System.out.println(pointtarget.toString());
    // // System.out.println("--------------");
    //
    // // Nächste Nachbarknoten finden
    // Collection<Point> neighborStart = this.network.nearestNeighbor(1, pointstart);
    // Collection<Point> neighborTarget = this.network.nearestNeighbor(1, pointtarget);
    //
    // Iterator<Point> poiA = neighborStart.iterator();
    // Node a = this.network.getNode(poiA.next().getCoordinate());
    //
    // Iterator<Point> poiB = neighborTarget.iterator();
    // Node b = this.network.getNode(poiB.next().getCoordinate());
    //
    // Path path = this.network.calculatePath(a, b);
    //
    // List<WayPoint> listWayPoints = this.network.pathToWayPointList(path, pointstart);
    //
    // this.wayPoints = listWayPoints;
    //
    // }
}
